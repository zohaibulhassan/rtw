...
class Pos extends MY_Controller{
                ...
                if($products == "" || count($products) == 0){
                    ...
                    foreach($products as $key => $row){
                        ...
                        if($pq->num_rows() > 0){
                            ...
                            $temp['subtotal'] = $qty[$key]*$product->mrp;;
                    ...
                    if(count($insertitems) > 0){
                        ...
                        $store['name'] = 'Orah Pharmacy';
                        $store['address1'] = 'Bahadurabad Branch';
                        $store['city'] = 'Karachi';
                        $store['phone'] = '0221321332';
        ...
        if($uq->num_rows() > 0){
            ...
            if($customer == ""){
                ...
                if($cq->num_rows() == 0){
                    ...
                    $insert['biller'] = 'Orah';
                    $insert['warehouse_id'] = 4;
                    $insert['order_discount'] = $this->input->post('discount');
                    $insert['shipping'] = $this->input->post('charges');
                    ...
                    if($products == "" || count($products) == 0){
                        ...
                        foreach($products as $key => $row){
                            ...
                            foreach($purchases as $purchase){
                                ...
                                $temp['discount'] = 0;
                                $temp['item_discount'] = 0;
                                $temp['subtotal'] = $qty[$key]*$purchase->mrp;
                        ...
                        if(count($insertitems) > 0){
                            ...
                            $payment = array(
                                ...
                                'amount' => $this->input->post('payamountval'),
                                'hold_amount' => 0,
    ...
    public function index_old($sid = null)
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_batch_method  = $_POST['batch_method'][$r];
                $item_batch_number  = !empty($_POST['batch_number'][$r]) ? $_POST['batch_number'][$r]:"";
               
                if($item_batch_method=="Manual"){
                    if (!$item_batch_number) {
                        $this->session->set_flashdata('error', lang('please_select_batch_no'));
                        admin_redirect('pos');
                    }
                }
                ...
                if (isset($item_code) && isset($real_unit_price) && isset($unit_price) && isset($item_quantity)) {
                    ...
                    if (isset($item_tax_rate) && $item_tax_rate != 0) {
                        $tax_details = $this->pos_model->getOrderTaxRateByID($item_tax_rate);
                    ...
                    // print_r($product);
                    // die;
                //     $batches = $this->getProductDataByCode($item_code, $warehouse_id, $customer_id);
                // if($batches){
                //      foreach ($batches as $batch) {
                //             $row->batches += $batch->batches;
                //         }
                // }
            ...
            $order_tax      = $this->pos_model->calculateOrderTax($this->input->post('order_tax'), ($total + $product_tax - $order_discount));
        ...
        if ($this->form_validation->run() == true && !empty($products) && !empty($data)) {
            ...
            if (($sid || $duplicate_sale) && $inv_items) {
                ...
                foreach ($inv_items as $item) {
                    ...
                    $pr[$ri] = ['id' => $c, 'item_id' => $row->id, 'label' => $row->name . ' (' . $row->code . ')',
                        'row'        => $row, 'combo_items' => $combo_items, 'batch_method' => $batch_method,'sus_batches' => $sus_batches, 'tax_rate' => $tax_rate, 'units' => $units, 'options' => $options, ];
                    $c++;
                }
            ...
            $this->data['warehouses']    = $this->pos_model->getWarehouseByIDorALL($this->session->userdata('warehouse_id'));
