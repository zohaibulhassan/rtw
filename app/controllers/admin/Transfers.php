...
class Transfers extends MY_Controller
{
    ...
    function add(){
        ...
        if ($this->form_validation->run()) {
            ...
            for ($r = 0; $r < $i; $r++) {
                ...
                $item_batch = $_POST['batch_number'][$r];
                ...
                $get_purchase_product_details = $this->transfers_model->getPurchaseProductDetails($product_details->id, $item_batch);
                $item_net_cost = $this->sma->formatDecimal($get_purchase_product_details->net_unit_cost);
                $unit_cost = $this->sma->formatDecimal($get_purchase_product_details->unit_cost);
                $real_unit_cost = $this->sma->formatDecimal($get_purchase_product_details->real_unit_cost);
    ...
    function delete($id = NULL)
    {
        ...
        if($counter_check > 0) {
            $this->sma->send_json(array('error' => 1, 'msg' => "Sorry you can not delete transfer because ".$check_purchase_transfer[$var_check]['product_name'] . " already sale"));
        } else {
            // echo "2";
