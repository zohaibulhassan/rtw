...
class Settings_model extends CI_Model
{
    ...
    public function getown_companiesByName($name)
    {
        $q = $this->db->get_where('own_companies', array('companyname' => $name), 1);
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
    ...
    public function add_own_companies($data)
    {
        if ($this->db->insert("own_companies", $data)) {
            return true;
        }
        return false;
    }

    public function add_own_companiess($data)
    {
        if ($this->db->insert_batch('own_companies', $data)) {
            return true;
        }
        return false;
    }

    public function updateown_companies($id, $data = array())
    {
        if ($this->db->update("own_companies", $data, array('id' => $id))) {
            return true;
        }
        return false;
    }

    // public function own_companiesHasProducts($own_companies_id)
    // {
    //     $q = $this->db->get_where('products', array('own_companies' => $own_companies_id), 1);
    //     if ($q->num_rows() > 0) {
    //         return $q->row();
    //     }
    //     return FALSE;
    ...
    public function deleteown_companies($id)
    {
        if ($this->db->delete("own_companies", array('id' => $id))) {
            return true;
        }
        return FALSE;
    }
    ...
    public function addBulkDiscount($data)
    {
        if ($this->db->insert('bulk_discount', $data)) {
            return TRUE;
        }
        return FALSE;
    }
    ...
    public function deleteBulkDiscount($id)
    {

        // echo $id;

    //    // $this->db->save_queries = TRUE;
        if ($this->db->delete('bulk_discount', array('id' => $id))) {
