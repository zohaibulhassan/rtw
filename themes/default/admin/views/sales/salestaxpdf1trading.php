        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="padding10">
                    ...
                    <div class="col-xs-4">
                        <strong>Invoice # : </strong> <?php echo $inv->reference_no; ?><br>
                        <strong> <?= lang('date'); ?>: </strong>  <?= date_format(date_create($inv->date),"Y-m-d"); ?><br>
                        <strong>Payment Term : </strong> <?= lang($inv->payment_terms); ?><br>
                        <strong>P.O Number : </strong> <?= lang($inv->po_number); ?><br>
                        <strong>P.O Date : </strong> <?= lang($inv->po_date); ?><br>
                        <strong>D.C # : </strong> <?= lang($inv->dc_num); ?><br>
                        ...
                        <tbody>
                            <?php
                                ...
                                foreach ($rows as $row):
                                    ...
                                    $my_row_subtotal = $this->sma->formatDecimal($two * $row->unit_quantity);
                            ...
                            <tr class="warning">
                                ...
                                <?php } ?>
                                <td style="width: 80px; text-align:center; vertical-align:middle;"><?= $this->sma->formatQuantity($row->quantity,2).' '.$row->product_unit_code; ?></td>
                                <td style="text-align:center; width:90px;"><?= $this->sma->formatMoney2($row->unit_price); ?></td>
                                <td style="text-align:center; width:90px;"><?= $this->sma->formatMoney2($row->mrp); ?></td>
                                <td style="text-align:center; width:90px;"></td>
                                <td style="text-align:center; width:90px;"><?= $this->sma->formatMoney2($row->tax_type); ?></td>
                                <?php
                                    if ($Settings->tax1 && $inv->product_tax > 0) {
                                        echo '<td style="text-align:center; vertical-align:middle;">' . ($row->item_tax != 0 ? '<small>('.($Settings->indian_gst ? $row->tax : $row->tax_code).')</small>' : '') . ' ' . $this->sma->formatMoney2($row->item_tax) . '</td>';
                                    }
                                    if ($Settings->product_discount && $inv->product_discount != 0) {
                                        echo '<td style="text-align:center; vertical-align:middle;">' . ($row->discount != 0 ? '<small>(' . $row->discount . ')</small> ' : '') . $this->sma->formatMoney2($row->item_discount) . '</td>';
                                    }
                                ?>
                                <td style="text-align:center; width:110px;"><?= $this->sma->formatMoney2($row->subtotal); ?></td>
                            </tr>
                            <?php
                                $r++;
                                endforeach;
                                ...
                                <div class="divTableCell pl-0 pr-0">
                                    <div class="divTable">
                                        <div class="divTableBody">
                                            ...
                                            <?php } ?>
                                            <div class="divTableRow">
                                                <div class="divTableCell bold">Cartage </div>
                                                <div class="divTableCell"><?php echo $this->sma->formatMoney2($inv->cartdiage); ?></div>
                                            </div>
                                            <div class="divTableRow">
                                                <div class="divTableCell bold">Net Amount</div>
                                                <div class="divTableCell"><?php echo $this->sma->formatMoney2($net_amount+$inv->cartdiage); ?></div>
                                            </div>
                                        </div>
                                    </div>
                                    <!-- DivTable.com -->
            ...
            <div class="clearfix"></div>
            <footer>
                <div class="pagenum-container">Footer <span class="pagenum"></span></div>
            </footer>
            <div class="clearfix"></div>
        </div>
    </div>
