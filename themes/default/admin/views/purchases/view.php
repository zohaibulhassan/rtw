            ...
            <ul class="btn-tasks">
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                        <?php if (count($return_rows) == 0) { ?>
                            <li>
                                <a href="<?= admin_url('purchases/purchase_return/' . $inv->id) ?>">
            ...
            <div class="col-lg-12">
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover table-striped print-table order-table">
                        ...
                        foreach ($rows as $row):
                            ...
                            <tr>
                                ...
                                <td style="vertical-align:middle;">
                                    ...
                                    <?= ($row->expiry && $row->expiry != '0000-00-00') ? '<br>' .lang('expiry').': ' . $row->expiry : ''; ?>
                                </td>
                                <?php if ($Settings->indian_gst) { ?>
                                ...
                                <td style="text-align:right; width:120px; padding-right:10px;"><?php $total2 = $row->net_unit_cost*$row->unit_quantity; echo $this->sma->formatMoney($total2); $totalamount = $totalamount+$total2; ?></td>
                                <?php
                                if ($Settings->tax1 && $inv->product_tax > 0) {
                        ...
                        if ($return_rows) {
                            $trow = $trow-1;
                            echo '<tr class="warning"><td colspan="'.$trow.'" class="no-border"><strong style="color:red;" >'.lang('returned_items').'</strong></td><td>'; ?>
                            <ul class="btn-tasks">
                                <li class="dropdown">
                                    <a data-toggle="dropdown" class="dropdown-toggle" href="#">
                                        <i class="icon fa fa-tasks tip" data-placement="left" title="<?= lang("actions") ?>"></i>
                                    ...
                                    <ul class="dropdown-menu pull-right tasks-menus" role="menu" aria-labelledby="dLabel">
                                        <li><a data-id="<?php echo $inv->id; ?>" style="cursor: pointer;" class="deletereturn"><i class="fa fa-trash"></i> Delete </a></li>
                                    </ul>
                                </li>
                            </ul>
                            ...
                            ?>
                                <tr class="warning">
                                    ...
                                    <td style="vertical-align:middle;">
                                        ...
                                        <?= '<br>' .lang('expiry').': ' . $row->expiry; ?>
                                    </td>
                                    <?php if ($Settings->indian_gst) { ?>
                                    <td style="width: 80px; text-align:center; vertical-align:middle;"><?= $row->hsn_code; ?></td>
                                    <?php } ?>
                                    <td style="width: 120px; text-align:center; vertical-align:middle;"><?= $this->sma->formatQuantity($row->quantity).' pcs'; ?></td>
                                    <?php
                                    if ($inv->status == 'partial') {
                                        echo '<td style="text-align:center;vertical-align:middle;width:120px;">'.$this->sma->formatQuantity($row->quantity).' pcs</td>';
                                    }
                                    ?>
                                    <td style="text-align:right; width:120px; padding-right:10px;"><?= $this->sma->formatMoney($row->net_unit_cost); ?></td>
                                    <td style="text-align:right; width:120px; padding-right:10px;"><?= $this->sma->formatMoney($row->net_unit_cost*$row->quantity); ?></td>
                                    <?php
                                    if ($row->item_tax > 0) {
                                        echo '<td style="width: 120px; text-align:right; vertical-align:middle;">' . $this->sma->formatMoney($row->item_tax) . '</td>';
                                    }
                                    ?>
                                    <td style="text-align:right; width:120px; padding-right:10px;"><?= $this->sma->formatMoney($row->adv_tax); ?></td>
                                    <?php
                        ...
                        <?php if ($inv->grand_total != $inv->total) { ?>
                            <tr>
                                ...
                                <td><?php echo $this->sma->formatMoney($totalamount) ;?></td>
                                <?php
                                if ($Settings->tax1 && $inv->product_tax > 0) {
                                ...
                                echo '<td style="text-align:right;">' . $this->sma->formatMoney($inv->total_adv_tax) . '</td>';
                                if ($Settings->product_discount && $inv->product_discount != 0) {
                                ...
                                ?>
                                <td style="text-align:right; padding-right:10px;"><?= $this->sma->formatMoney($return_purchase ? (($inv->total + $inv->product_tax)+($return_purchase->total + $return_purchase->product_tax)) : ($inv->total + $inv->product_tax+$inv->total_adv_tax)); ?></td>
                            </tr>
                        <?php } ?>
                        <?php
                        if ($return_purchase) {
                        ...
                        <?php if ($inv->order_discount != 0) {
                            echo '<tr><td colspan="' . $col+2 . '" style="text-align:right; padding-right:10px;;">' . lang("order_discount") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">'.($inv->order_discount_id ? '<small>('.$inv->order_discount_id.')</small> ' : '') . $this->sma->formatMoney($return_purchase ? ($inv->order_discount+$return_purchase->order_discount) : $inv->order_discount) . '</td></tr>';
                        }
                        ?>
                        <?php if ($Settings->tax2 && $inv->order_tax != 0) {
                            echo '<tr><td colspan="' . $col+2 . '" style="text-align:right; padding-right:10px;">' . lang("order_tax") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->sma->formatMoney($return_purchase ? ($inv->order_tax+$return_purchase->order_tax) : $inv->order_tax) . '</td></tr>';
                        }
                        ?>
                        <?php if ($inv->shipping != 0) {
                            echo '<tr><td colspan="' . $col+2 . '" style="text-align:right; padding-right:10px;;">' . lang("shipping") . ' (' . $default_currency->code . ')</td><td style="text-align:right; padding-right:10px;">' . $this->sma->formatMoney($inv->shipping) . '</td></tr>';
                        }
                        ?>
                        <tr>
                            <td colspan="<?= $col+2; ?>"
