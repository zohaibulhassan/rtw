$(document).ready(function() {
                    ...
                    if (this.id == pr_tax) {
                        if (this.type == 1) {
                            ...
                            if (poitems[item_id].row.tax_method == 0) {
                                // alert(this.rate + "--"+ real_unit_cost);
                                pr_tax_val = formatDecimal((((real_unit_cost) * parseFloat(this.rate)) / (100 + parseFloat(this.rate))), 4);
                                pr_tax_rate = formatDecimal(this.rate) + '%';
                                net_cost -= pr_tax_val;
                            } else {
                                // alert(this.rate + "--"+ real_unit_cost);
                                pr_tax_val = formatDecimal((((real_unit_cost) * parseFloat(this.rate)) / 100), 4);
                                // alert(pr_tax_val);
                                pr_tax_rate = formatDecimal(this.rate) + '%';
                            }
    ...
    $(document).on("change", '.discount_one, .discount_two, .discount_three', function() {
        ...
        if ($(this)[0].checked == true) {
            ...
            // fed_tax_checked = "true";

            consiment_cost = poitems[item_id].row.consiment;
            old_discount = JSON.parse(poitems[item_id].row.discount);

            discount_one = $(this).val();
            total_discount_one = (consiment_cost * discount_one) / 100;
            tax_value = formatMoney(((parseFloat(poitems[item_id].row.cost)) * parseFloat(poitems[item_id].row.base_quantity) * parseFloat(poitems[item_id].tax_rate.rate)) / 100);
            total_cost = formatMoney(((parseFloat(poitems[item_id].row.cost) * parseFloat(poitems[item_id].row.base_quantity)) + (parseFloat(tax_value))));
            discount_with_qty += total_discount_one * parseFloat(poitems[item_id].row.base_quantity);

            poitems[item_id].row.discount = JSON.stringify(discount_with_qty + old_discount);

        } else {

            discount_one_checked = "false";
            // fed_tax_checked = "false";
            consiment_cost = poitems[item_id].row.consiment;
            old_discount = JSON.parse(poitems[item_id].row.discount);
            ...
            poitems[item_id].row.discount = JSON.stringify(old_discount - discount_with_qty);
        ...
        if (get_class == 'discount_one_checked') {
            poitems[item_id].row.discount_one_checked = discount_one_checked;
        } else if (get_class == 'discount_two_checked') {
            poitems[item_id].row.discount_two_checked = discount_one_checked;
        } else if (get_class == 'discount_three_checked') {
            poitems[item_id].row.discount_three_checked = discount_one_checked;
        }
        // else if(get_class == 'fed_tax_checked'){
        //     poitems[item_id].row.fed_tax_checked = fed_tax_checked;
        // }
    ...
    $(document).on("focus", '.rbatch_number', function() {}).on("change", '.rbatch_number', function() {
        ...
        poitems[item_id].row.batch = new_batch_number;
        localStorage.setItem('poitems', JSON.stringify(poitems));
    ...
    $(document).on("focus", '.rprod_expiry', function() {
        ...
        poitems[item_id].row.expiry = new_prod_expiry;
        localStorage.setItem('poitems', JSON.stringify(poitems));
    ...
    // $(document).on("focus", '.fed_tax', function () {
    //     old_row_qty = $(this).val();
    // }).on("change", '.fed_tax', function () {
    ...
    //     //     return;
    //     // }
    //     var new_fed_tax = parseFloat($(this).val()),
    //     item_id = row.attr('data-item-id');
    //     poitems[item_id].row.fed_tax_rate = new_fed_tax;
    //     // if(poitems[item_id].row.fed_tax_rate) {
    //     //     $.each(poitems[item_id].units, function(){
    ...
    //     // discount_with_qty_1 = 0;
    //     // discount_with_qty_2 = 0;
    //     // discount_with_qty_3 = 0;

    //     // if(JSON.parse(poitems[item_id].row.discount_one_checked)  == true){
    //         // fed_tax = (((poitems[item_id].row.consiment * (poitems[item_id].row.discount_one)) / 100) * poitems[item_id].row.base_quantity);
    //     // } 
    ...
    $(document).on("focus", '.rquantity', function() {
        ...
        if (JSON.parse(poitems[item_id].row.discount_one_checked) == true) {
            discount_with_qty_1 = (((poitems[item_id].row.consiment * (poitems[item_id].row.discount_one)) / 100) * poitems[item_id].row.base_quantity);
        }
        if (JSON.parse(poitems[item_id].row.discount_two_checked) == true) {
            discount_with_qty_2 = (((poitems[item_id].row.consiment * (poitems[item_id].row.discount_two)) / 100) * poitems[item_id].row.base_quantity);
        }
        if (JSON.parse(poitems[item_id].row.discount_three_checked) == true) {
            discount_with_qty_3 = (((poitems[item_id].row.consiment * (poitems[item_id].row.discount_three)) / 100) * poitems[item_id].row.base_quantity);
            ...
            tax_value = 0;
            total_cost = 0;
            poitems[item_id].row.discount = 0;
        }

        poitems[item_id].row.discount = JSON.stringify(discount_with_qty_1 + discount_with_qty_2 + discount_with_qty_3);
...
function loadItems() {
    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(sortedItems, function() {
            ...
            // gst_tax_rate = item.row.gst_tax_rate;
            further_tax_rate = item.row.further_tax_rate;
            ...
            var row_no = item.id;
            var advtax = ((item_cost+pr_tax_val)*item_qty)/100*item.row.adv_tax_for_purchase;

            var newTr = $('<tr id="row_' + row_no + '" class="row_' + item_id + '" data-item-id="' + item_id + '"></tr>');
            tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><input name="part_no[]" type="hidden" class="rpart_no" value="' + item_supplier_part_no + '"><input name="product_rowid[]" type="hidden" value="' + row_no + '"><span class="sname" id="name_' + row_no + '">' + item_code + ' - ' + item_name + (sel_opt != '' ? ' (' + sel_opt + ')' : '') + ' <span class="label label-default">' + item_supplier_part_no + '</span></span> </td>';
            // if (site.settings.product_expiry == 1) {
            //     tr_html += '<td><input class="form-control date rexpiry" name="expiry[]" type="text" autocomplete="off" value="' + item_expiry + '" data-id="' + row_no + '" data-item="' + item_id + '" id="expiry_' + row_no + '"></td>';
            // }
            ...
            tr_html += '<td class="text-right"><span class="text-right scost" id="scost_' + row_no + '">' + formatMoney(item.row.mrp) + '</span></td>';


            tr_html += '<td><input name="quantity_balance[]" type="hidden" class="rbqty" value="' + item_bqty + '"><input class="form-control text-center rquantity" name="quantity[]" type="text" autocomplete="off" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" value="' + formatQuantity2(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"><input name="product_unit[]" type="hidden" class="runit" value="' + product_unit + '"><input name="product_base_quantity[]" type="hidden" class="rbase_quantity" value="' + base_quantity + '"></td>';

            tr_html += '<td><input class="form-control text-center rbatch_number" value="' + ((item.row.batch !== null && item.row.batch !== '') ? item.row.batch : "") + '" name="batch_number[]" type="text" autocomplete="off" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" value="" data-id="' + row_no + '" data-item="' + item_id + '" id="batch_number_' + row_no + '" onClick=""></td>';

            tr_html += '<td><input class="form-control text-center date rprod_expiry" value="' + ((item.row.expiry !== null && item.row.expiry !== '') ? item.row.expiry : "") + '" name="expiry[]" type="text" autocomplete="off" tabindex="' + ((site.settings.set_focus == 1) ? an : (an + 1)) + '" value="" data-id="' + row_no + '" data-item="' + item_id + '" id="podate_' + item_id + '" onClick=""></td>';


            tr_html += '<td> <input id="discount_one_' + row_no + '"  class="discount_one" name="discount_one_' + my_count + '" ' + (((JSON.parse(item.row.discount_one_checked) == true) ? "checked" : "")) + '  type="checkbox" value="' + (((item.row.discount_one)) ? (item.row.discount_one).slice(0, 5) : 0) + '" onClick="this.select();"  data-id="' + row_no + '" data-item="' + item_id + '" > ( ' + (((item.row.discount_one)) ? (item.row.discount_one).slice(0, 5) : 0) + '% ) <br> ' + formatMoney(((item.row.consiment * (item.row.discount_one)) / 100) * item.row.base_quantity) + '  </td>';

            tr_html += '<td><input id="discount_two_' + row_no + '"  class="discount_two" name="discount_two_' + my_count + '" ' + ((JSON.parse(item.row.discount_two_checked) == true) ? "checked" : "") + ' type="checkbox" value="' + (((item.row.discount_two)) ? (item.row.discount_two).slice(0, 5) : 0) + '" onClick="this.select();"  data-id="' + row_no + '" data-item="' + item_id + '" > ( ' + (((item.row.discount_two)) ? (item.row.discount_two).slice(0, 5) : 0) + '% ) <br> ' + formatMoney(((item.row.consiment * (item.row.discount_two)) / 100) * item.row.base_quantity) + '  </td>';


            tr_html += '<td><input id="discount_three_' + row_no + '"  class="discount_three" name="discount_three_' + my_count + '" ' + ((JSON.parse(item.row.discount_three_checked) == true) ? "checked" : "") + ' type="checkbox" value="' + (((item.row.discount_three)) ? (item.row.discount_three).slice(0, 5) : 0) + '" onClick="this.select();"  data-id="' + row_no + '" data-item="' + item_id + '" > ( ' + (((item.row.discount_three)) ? (item.row.discount_three).slice(0, 5) : 0) + '% ) <br> ' + formatMoney(((item.row.consiment * (item.row.discount_three)) / 100) * item.row.base_quantity) + '  </td>';


            // tr_html += '<td><input id="fed_tax_' + row_no + '"  class="fed_tax" name="fed_tax_' + my_count + '" ' + ((JSON.parse(item.row.fed_tax_checked) == true) ? "checked" : "" ) + ' type="checkbox" value="'+ (((item.row.fed_tax_rate)) ? (item.row.fed_tax_rate).slice(0,5) : 0) +'" onClick="this.select();"  data-id="' + row_no + '" data-item="' + item_id + '" > ( '+ (((item.row.fed_tax_rate)) ? (item.row.fed_tax_rate).slice(0,5) : 0) +'% ) <br> '+  formatMoney(((item.row.mrp * (item.row.fed_tax_rate)) / 100) * item.row.base_quantity)  +'  </td>';


            tr_html += '<td><input id="fed_tax_' + row_no + '"  class="form-control input-sm text-right fed_tax" name="fed_tax_' + my_count + '" type="text" autocomplete="off" value="' + (((item.row.fed_tax_rate)) ? (item.row.fed_tax_rate) : 0) + '" onClick="this.select();"  data-id="' + row_no + '" data-item="' + item_id + '" readonly > </td>';

            // tr_html += '<td><input id="gst_tax_' + row_no + '"  class="form-control input-sm text-right gst_tax" name="gst_tax_' + my_count + '" type="text" autocomplete="off" value="'+ (((item.row.gst_tax_rate)) ? (item.row.gst_tax_rate) : 0) +'" onClick="this.select();"  data-id="' + row_no + '" data-item="' + item_id + '" readonly > </td>';
            tr_html += '<td><input id="further_tax_' + row_no + '"  class="form-control input-sm text-right further_tax" name="further_tax_' + my_count + '" type="text" autocomplete="off" value="' + (((item.row.further_tax_rate)) ? (item.row.further_tax_rate) : 0) + '" onClick="this.select();"  data-id="' + row_no + '" data-item="' + item_id + '" readonly > </td>';
            tr_html += '<td><input id="advtax' + row_no + '"  class="form-control input-sm text-right advtax" name="advtax' + my_count + '" type="text" autocomplete="off" value="' + advtax + '" onClick="this.select();"  data-id="' + row_no + '" data-item="' + item_id + '" readonly > </td>';
            ...
            if (po_edit) {
                tr_html += '<td class="rec_con"><input name="ordered_quantity[]" type="hidden" class="oqty" value="' + item_oqty + '"><input class="form-control text-center received" name="received[]" type="text" autocomplete="off" value="' + formatDecimal(unit_qty_received) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="received_' + row_no + '" onClick=""><input name="received_base_quantity[]" type="hidden" class="rrbase_quantity" value="' + qty_received + '"></td>';
            }
            if (site.settings.product_discount == 1) {
                tr_html += '<td class="text-right"><input class="form-control input-sm rdiscount" name="product_discount[]" type="hidden" id="discount_' + row_no + '" value="' + item_ds + '"><spaan class="text-right sdiscount text-danger" id="sdiscount_' + row_no + '">' + formatMoney(0 - (item_ds)) + '</span></td>';
            }
            if (site.settings.tax1 == 1) {
                tr_html += '<td class="text-right"><input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' + row_no + '" value="' + pr_tax.id + '"><span class="text-right sproduct_tax" id="sproduct_tax_' + row_no + '">' + (pr_tax_rate ? '(' + pr_tax_rate + ')' : '') + ' ' + formatMoney(pr_tax_val * item_qty) + '</span><input class="form-control input-sm text-right tax_value" name="product_tax_value[]" type="hidden" id="product_tax_value' + row_no + '" value="' + (pr_tax_rate ? (formatMoney(pr_tax_val * item_qty)) : '') + '"></td>';
            }
            tr_html += '<td class="text-right"> <input class="form-control input-sm text-right rssubtotal" name="ssubtotal[]" type="hidden" id="ssubtotal_' + row_no + '" value="' + formatMoney(((parseFloat(item_cost) + parseFloat(pr_tax_val)) * parseFloat(item_qty) + +advtax+(parseFloat(fed_tax_rate) * parseFloat(item_qty)))) + '"> <span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney((((parseFloat(item_cost) + parseFloat(pr_tax_val)) * parseFloat(item_qty)) - item_ds) + advtax + (parseFloat(fed_tax_rate) * parseFloat(item_qty))) + '</span></td>';
            tr_html += '<td class="text-center"><i class="fa fa-times tip podel" id="' + row_no + '" title="Remove" style="cursor:pointer;"></i></td>';
            newTr.html(tr_html);
            newTr.prependTo("#poTable");
            total += formatDecimal((((parseFloat(item_cost) + parseFloat(pr_tax_val)) * parseFloat(item_qty) - item_ds) +advtax+ (parseFloat(fed_tax_rate) * parseFloat(item_qty))), 4);
        ...
        var tfoot = '<tr id="tfoot" class="tfoot active"><th colspan="' + col + '">Total</th><th class="text-center">' + formatQty(parseFloat(count) - 1) + '</th> <th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th><th class="text-center"></th>';
        if (po_edit) {
