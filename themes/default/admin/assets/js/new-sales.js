...
function loadItems() {
    ...
    if (localStorage.getItem('slitems')) {
        ...
        $.each(sortedItems, function() {
            ...
            tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_price) + ((customer_type == "consignment") ? parseFloat(pr_tax_val) : 0)) * parseFloat(item_qty) - item_ds) + (((item.gst_no == "") && (customer_type == "consignment") && (item.tax_rate.type == 1)) ? '111' : ((inclusive_price_calculation + ((customer_type == "consignment") ? item.row.get_selected_fed_tax_rate : 0)) * parseFloat(item_qty)))) + '</span></td>';
